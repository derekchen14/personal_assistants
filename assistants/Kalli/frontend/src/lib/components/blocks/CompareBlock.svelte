...
<div class="flex flex-col flex-1 min-h-0">
    {#if content}
        <div class="px-4 py-3 border-b border-[var(--color-border)] bg-[var(--color-surface-hover)] text-sm leading-relaxed">
            {@html md(content)}
        </div>
    {/if}
    ...
    <div class="grid grid-cols-2 gap-4 p-4 flex-1 min-h-0">
        {#each [left, right] as panel}
            <div class="flex flex-col border border-[var(--color-border)] rounded-lg overflow-hidden min-h-0">
                <div class="px-3 py-2 bg-[var(--color-surface-hover)] border-b border-[var(--color-border)] shrink-0">
                    <h4 class="text-sm font-semibold text-[var(--color-secondary)] truncate">
                        {panel.title || 'Untitled'}
                    </h4>
                    {#if panel.status}
                        <span class="text-xs text-[var(--color-text-muted)]">{panel.status}</span>
                    {/if}
                </div>
