"""
Slot types for Kalli (Onboarding).

12 universal slots + 2 domain-specific.
Grounding entity_parts: assistant, requirement, spec, tool.

Hierarchy:
  BaseSlot
  ├── GroupSlot            (multiple values in a list)
  │   ├── SourceSlot       (existing entities: assistant, requirement, spec)
...
class SourceSlot(GroupSlot):
  """References existing entities. Each entity is {ast, req, spec, tool, ver}."""
  ...
  def add_one(self, ast, req='', spec='', tool='', ver=False):
    key = f"{ast}-{req}"
    alt_key = f"{self.active_ast}-{req}"
    if key in self._keys:
      idx = self._keys.index(key)
      self.values[idx]['ver'] = ver
    elif alt_key in self._keys:
      pass  # earlier assistant is the active one
    else:
      entity = {'ast': ast, 'req': req, 'spec': spec, 'tool': tool, 'ver': ver}
...
class TargetSlot(SourceSlot):
  """New entities being created (new assistant name, new requirement)."""
...
class RemovalSlot(SourceSlot):
  """Entities to remove from the build."""
