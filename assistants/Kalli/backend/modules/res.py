...
class RESResult:
    ...
    def __init__(self, message: str, raw_utterance: str,
                 interaction: dict | None = None,
                 actions: list | None = None,
                 code_snippet: dict | None = None,
...
class RES:
    ...
    def _build_response(self, text: str, pex_result: PEXResult,
        ...
        code_snippet = None
        for entry in pex_result.tool_log:
            if entry.get('tool') == 'python_execute':
                result = entry.get('result', {})
                if result.get('status') == 'success':
                    code_snippet = {
                        'source': 'python_execute',
                        'snippet': entry.get('input', {}).get('code', ''),
                    }
                    break
