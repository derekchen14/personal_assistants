...
_SHORTCUTS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r'\bwhat.*(next|now)\b', re.I), 'next_step'),
...
class NLU:
    ...
    def _tally_votes(self, votes: list[dict]) -> NLUResult:
        ...
        # Find the flow with the most votes
        ...
        # Average confidence across agreeing voters
        avg_confidence = sum(
            float(v.get('confidence', 0.5)) for v in best_votes
        ) / len(best_votes)

        # Boost confidence on unanimous agreement
        ...
        # Merge slots from the highest-confidence agreeing vote
