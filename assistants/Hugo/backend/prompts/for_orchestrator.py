...
INTENT_TAXONOMY = (
    ...
    'which flow to activate when the mapping is obvious (a click or a clear continuation). When '
    'you consult NLU with `understand`, pass your sense as `intent`: Research / Draft / Revise / '
    'Publish narrow its detection; Plan, Clarify, and Converse offer no real signal, so NLU '
    'ignores them and searches the full flow ontology. When you '
...
TOOL_POLICY = (
    ...
    '**Creating a new post.** A Draft opener that starts a brand-new post — the user names a post '
    'to write but none exists yet (belief carries a title with no id, or a single `find_posts` '
    'returns nothing) — needs a post to write into. Call `create_post` with the title FIRST, then '
    'stack the drafting flow on it. `create_post` only makes an empty post without content, the '
    'flow you stack still does all the actual writing.\n'
    '**A flow that stalls on a missing entity.** When a flow you dispatched returns a `partial` '
