...
TOOL_POLICY = (
    ...
    '**Decide by intent.** Act on the intent NLU wrote:\n'
    ...
    'unanswered.\n'
    '**Staging and dispatching flows.** To run a flow:\n'
    '1. `write_state` op=stackon with the flow_name.\n'
    "2. `write_state` op=update_flow with fields.slots set from belief's `pred_slots` — pass "
    'the typed values through unchanged; never rename, drop, or invent a slot. Skip this step '
