...
Publishes the post to one or more channels (defaults to the primary blog).
The skill iterates each channel: channel_status → release_post, then emits
a single JSON response with a `releases` array.
"""
