...
1. Read belief (`read_state`). Belief carries NLU's detection for this turn.
...
5. Stack on ALL the flows AT ONCE, in reverse execution order: push the LAST flow first and the
   FIRST flow last, so the first sits on top. Every push is `write_state` op=stackon; only the
   final push (the flow to run now) carries `active: true`. The stack now holds the whole plan —
   it is observable by every agent and survives even if you lose track of the plan later.
6. After each flow completes, `pop_completed` surfaces the next flow at the top. Judge whether the
   user's goal is met: if not, run the surfaced flow with `activate_flow`; if it is, stop and
