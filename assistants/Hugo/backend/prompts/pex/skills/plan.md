...
5. Stack on ALL the flows AT ONCE, in reverse execution order: push the LAST flow first and the
   FIRST flow last, so the first sits on top. Every push is `manage_flows` op="stackon"; only the
...
6. After each flow completes, `manage_flows` op="pop" removes Completed and Invalid flows all at
   once, surfacing the next flow at the top. Judge whether the user's goal is met: if not, run the
   surfaced flow with `manage_flows` op="activate"; if it is, stop and report what was accomplished.
