...
1. Read the post identifier and channel list from `<resolved_details>`. The channel list is an array of channel names such as `['Substack']` or `['Substack', 'LinkedIn']`. When neither the resolved details nor the user names a channel, default to the primary blog: `['mt1t']` — do NOT ask which channel to use. Process each channel independently so that one channel's failure does not abort the rest.

2. For each channel in the list, in order (`platform` values are lowercase ids: `mt1t`, `substack`, `linkedin`, `twitter`):
  a. Call `channel_status(post_id=<id>, platform=<name>)` to verify the channel is authenticated and reachable
...
3. When the status call completes, call `release_post(post_id=<id>, platform=<name>)` to publish.
...
Trajectory:
1. `channel_status(post_id='abcd0123', platform='substack')` → `{_success: True, ok: True}`.
2. `release_post(post_id='abcd0123', platform='substack')` → `{_success: True, url: 'https://substack.com/p/user-simulators-for-training-rl-agents'}`.
...
Trajectory:
1. `channel_status(post_id='abcd0123', platform='substack')` → ok.
2. `release_post(post_id='abcd0123', platform='substack')` → `{_success: True, url: 'https://substack.com/p/user-simulators'}`.
3. `channel_status(post_id='abcd0123', platform='linkedin')` → `{_success: False, _error: 'auth_expired'}`.
