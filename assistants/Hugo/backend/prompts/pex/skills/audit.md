...
1. Decide which audit tools to run based on the latest user message in `<recent_conversation>` and the resolved details. The three tool families:
   a. **`editor_review(post_id)`** — voice and AI-slop checks (em-dashes, fragments, fillers, vague attributions). Default-on.
   b. **`inspect_post(post_id)`** — structural metrics (heading hierarchy, section word counts, paragraph density). Default-on.
   c. **`find_posts(status='published', count=<reference_count>)` + `compare_style(post_id, references=<ref_ids>)`** — voice comparison against prior published posts. Opt-in: only run when the user asks for comparison ("against my last few posts", "compare my voice"), or when `reference_count` is set in the resolved details.
...
- `compare_style(post_id, references)` voice-comparison tool. Returns a structured report on sentence-length, vocabulary, and paragraph metrics. Run only when the user asks for comparison.
- `editor_review(post_id)` applies the fixed editorial style guide. Default-on for generic audits; the primary source of AI-slop findings.
...
Trajectory:
1. `editor_review(post_id='abcd0123')` → flags two filler transitions ("Let's unpack this" twice). Threshold is 0.
2. `inspect_post(post_id='abcd0123')` → no structural anomalies.
3. `find_posts(status='published', count=3)` → returns ref_a, ref_b, ref_c.
4. `compare_style(post_id='abcd0123', references=['ref_a', 'ref_b', 'ref_c'])` → average sentence length is 31 words vs. the user's 18-word baseline; vocabulary density skews 12% more abstract than the references.
