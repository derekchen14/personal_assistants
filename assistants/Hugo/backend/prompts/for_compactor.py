"""
Compression summary prompt — the middle-summarizer for the context compactor: a reference-only
handoff prefix, a structured checkpoint template, and an iterative-update path that folds new
turns into the previous summary on re-compaction. The summarizer runs through PromptEngineer on
the LOW tier (a cheap auxiliary model).
"""

import json

# Handoff prefix for a compacted context. Hugo's persistent memory lives in the frozen
# three-tier system prompt (no MEMORY.md / USER.md; preferences live in the prompt).
SUMMARY_PREFIX = (
...
# verbatim '## Active Task' quote of a past request as fresh input.
END_OF_SUMMARY = ('\n\n--- END OF CONTEXT SUMMARY — '
                  'respond to the message below, not the summary above ---')

# Filter-safe summarizer preamble: prior turns are source material, never requests.
_PREAMBLE = (
...
# Structured checkpoint template targeting Hugo's entities (posts / sections / snippets /
# channels).
...
# Truncation limits for the summarizer input: the budget being protected here is the LOW-tier
# summary model's context window, not the orchestrator's.
_CONTENT_MAX = 6000
_CONTENT_HEAD = 4000
_CONTENT_TAIL = 1500
_TOOL_ARGS_MAX = 1500
...
def _serialize_for_summary(middle:list[dict]) -> str:
    ...
    for the summary to preserve specifics."""
    parts = []
