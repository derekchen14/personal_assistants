...
def test_find_single_item_emits_list_block_only(monkeypatch):
...
def test_find_scratchpad_write_shape(monkeypatch):
    ...
    comps['flow_stack'].stackon('find')
    top = comps['flow_stack'].get_flow()
    top.slots['query'].add_one('aviation')

    monkeypatch.setattr(ResearchPolicy, '_expand_query',
        lambda self, query: ['aviation'])

    items = [
        {'post_id': 'p1', 'title': 'Flight', 'status': 'published',
         'preview': 'first snippet'},
        {'post_id': 'p2', 'title': 'Gliders', 'status': 'draft',
         'preview': 'second snippet'},
    ]
    state = make_state()
    ...
    assert pad['query'] == 'aviation'
    assert len(pad['items']) == 2
...
def test_find_dedupes_across_expanded_terms(monkeypatch):
    ...
    post_id. One list block; post_ids are unique."""
    policy, comps = build_policy('find')
    comps['flow_stack'].stackon('find')
    top = comps['flow_stack'].get_flow()
    ...
    # Three calls to find_posts, each returning overlapping items.
    first_call = [
