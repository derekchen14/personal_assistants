...
Run:
  pytest utils/evals/e2e_agent_evals.py -v -s --tb=short
  pytest utils/evals/e2e_agent_evals.py -v -s --tb=short --strict        # also check tool order
  pytest utils/evals/e2e_agent_evals.py -v -s --tb=short --eval-verbose  # save report to disk
"""
...
from utils._snapshot import assert_snapshot, project_state
...
def _run_with_retry_on_flake(tester, step_def, request):
    ...
    """
    try:
        from utils.evals.playwright_evals.dump import write_failure_dump
    except ImportError:
    ...
    reproducer = (
        f'pytest utils/evals/e2e_agent_evals.py::{cls_name}'
...
def _progress_snapshot() -> str:
    ...
    Format:
      ...
      [progress] no progress file — run pytest utils/evals/e2e_agent_evals.py
      [progress] run complete — N/42 passed in Xm YYs
    """
    if not _PROGRESS_PATH.exists():
        return '[progress] no progress file — run pytest utils/evals/e2e_agent_evals.py'
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == '--progress':
        ...
        sys.exit(0)
    print('Usage: python utils/evals/e2e_agent_evals.py --progress')
    sys.exit(1)
