...
pytest utils/evals/e2e_agent_evals.py -v --tb=short

# E2E multi-turn ambiguity (real LLM, ~2 min, 3 scenarios)
pytest utils/evals/e2e_multiturn_evals.py -v --tb=short

# E2E with reliability checks (runs each turn 3x, slower)
pytest utils/evals/e2e_agent_evals.py -v -m reliability
```
...
- **Intentional behavior change** (e.g., a policy now emits a `card` block where it previously emitted a `toast`): re-run with `UPDATE_SNAPSHOTS=1 pytest utils/evals/e2e_agent_evals.py` and explain the diff in the PR body.
...
4. Run `pytest utils/evals/e2e_agent_evals.py -v -k "conv_XXXX"` to test just the new case
