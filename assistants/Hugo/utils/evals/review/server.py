...
The scenarios under utils/evals/datasets/scenarios/ are the DB; this serves a small UI to read each
conversation and save a per-conversation verdict + comment under datasets/feedback/.
  ...
  GET  /api/scenario/<id>   -> datasets/scenarios/<id>.json
  GET  /api/feedback/<id>   -> datasets/feedback/<id>.json (stub if none)
  PUT  /api/feedback/<id>   -> save datasets/feedback/<id>.json

Run from the Hugo directory:
    python utils/evals/review/server.py
...
PORT = 8023
ROOT = Path(__file__).resolve().parent                 # utils/evals/review
SCENARIOS = ROOT.parent / 'datasets' / 'scenarios'     # utils/evals/datasets/scenarios
FEEDBACK = ROOT.parent / 'datasets' / 'feedback'


def _scan_kind(case):
    # Conversation-level no-flow intent: 'Plan' or 'Clarify' if any user turn carries it, else None.
    for turn in case.get('turns', []):
        labels = turn.get('labels') or {}
        if labels.get('intent') in ('Plan', 'Clarify') or labels.get('flow') == 'Clarify':
            return 'Clarify' if labels.get('flow') == 'Clarify' else labels['intent']
    return None
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _item(self, case, fp):
        cid = case.get('convo_id', fp.stem)
    ...
    def _scenario_list(self):
        # Main review log: only cases flagged for review (one per batch is seeded on the scenario file).
        items = []
        for fp in sorted(SCENARIOS.glob('*.json')):
            case = json.loads(fp.read_text())
            if case.get('flagged', False):
                items.append(self._item(case, fp))
        return items
