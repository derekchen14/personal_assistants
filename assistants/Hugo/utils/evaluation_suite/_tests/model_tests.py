...
decision, no full PEX loop (that trajectory view is the Traces tier's job, `_traces/run_traces.py`).
  ...
  python utils/evaluation_suite/_tests/model_tests.py --module nlu       # NLU flow-detection accuracy
  python utils/evaluation_suite/_tests/model_tests.py --module nlu,pex   # several
...
from utils.evaluation_suite.harness import _build_agent, all_ids, load_cases, sample


def _load_cases(limit:int=0) -> list:
    """`--limit N` takes the first N train cases (ad-hoc); no limit → a fresh dev sample of 8."""
    return load_cases(all_ids()[:limit]) if limit else load_cases(sample())
