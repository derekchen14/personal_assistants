...
def _score_convo(case:dict, domain_tools:set, judge:bool=False) -> dict:
...
def main():
    ...
    if args.ids:
        ...
        ids = sample(args.sample)                         # dev = a fresh random sample
    cases = load_cases(ids)

    domain_tools = _domain_tools()
    results = [_score_convo(case, domain_tools, args.judge_response) for case in cases]
    for cid, res in zip(ids, results):
        line = ' '.join(f'{name}={res[name]}' for name in _CRITERIA)
        print(f"{cid}: {line} | {res['latency_secs']}s")
    print(f"\n== {len(cases)} convos ==")
    print(' '.join(f'{name}={_mean([r[name] for r in results if r[name] is not None])}'
                   for name in _CRITERIA)
          + f" latency_mean={_mean([r['latency_secs'] for r in results])}s")
