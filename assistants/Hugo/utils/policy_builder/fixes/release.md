...
### `update_post(status='published')` is gated on tool success (AD-6 error artifact)
...
- **Why:** Inventory § Known gaps #3 and `_theme4_feedback.md § release` both flagged this as disk-state corruption — step 14 of the eval explicitly expects `channel_status` / `release_post` to fail. Per AD-6 Section 1, tool-call failure is **not** ambiguity; it is an error artifact with `metadata.tool_error` and the error text in `code`.
...
### Eval rubric aligned with AD-6 error artifact
...
- **What changed:** The eval spec for step 14 (`utils/tests/e2e_agent_evals.py`) dropped `expected_ambiguity={'channel_unavailable'}` and added:
  ...
  `_check_level1` now reads `artifact.origin` and `artifact.parts.get('tool_error')` and asserts both. A short-circuit was added so error-origin frames don't fail the "empty response" check (their payload is in `metadata` / `code`, not in blocks).
...
- **AD-5** (terminology) — policy "scans the tool_log", "returns an error artifact"; no "fires" / "triggers" language introduced.
- **AD-6** (three failure modes, three distinct channels):
  - **Tool-call failure** → `TaskArtifact(origin='error', metadata={tool_error, channel, post_id, reason}, code=err_msg)`. Not ambiguity.
