...
from utils.tests.parity.capture_oracle import _clean_leftovers
from utils.tests.parity.trace_recorder import extract_trace, render_sidecar

TRACES_DIR = _HUGO_ROOT / 'utils' / 'tests' / 'traces'
...
TRAJECTORIES = [
    {
        'name': 'draft_create',
        'description': ('Clean single-flow **Draft** turn: create a new post from a topic. '
                        'Expected shape: detect_and_fill, stack + fill the create flow via '
                        'write_state, activate_flow(create) with a completion record, then a '
                        'direct reply.'),
...
def _build_agent():
    """Orchestrator-path Agent: debug=True (same as the parity runners)."""
...
def _seed_post(post_id:str, title:str, sections:dict):
    ...
    created = PostService().create_post(title, sections=list(sections), post_id=post_id)
...
def _record(spec:dict, idx:int) -> dict:
    ...
    for key, value in spec.get('prefs', {}).items():
        agent.memory.write_preference(key, value)  # pre-session → frozen into the prompt
