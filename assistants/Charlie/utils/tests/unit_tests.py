...
# ═══════════════════════════════════════════════════════════════════
# MemoryManager — file-backed scratchpad JSONL (changes.md §5.3)
# ═══════════════════════════════════════════════════════════════════

class TestMemoryManager:
    """L1 scratchpad in both modes: the legacy in-memory dict (no path — old pipeline) and
    the append-only JSONL file (path set — orchestrator substrate). Covers writer stamping,
    filtering, clear/truncate, and the completion-record shape."""

    @pytest.fixture
    def file_memory(self, minimal_config, tmp_path):
        return MemoryManager(minimal_config, scratchpad_path=str(tmp_path / 'scratchpad.jsonl'))

    # ── in-memory mode (old pipeline, unchanged contract) ────────────

    def test_inmemory_mode_unchanged(self, minimal_config):
        memory = MemoryManager(minimal_config)
        memory.write_scratchpad('repair', 'bad outline')
        assert memory.read_scratchpad('repair') == 'bad outline'
        assert memory.read_scratchpad() == {'repair': 'bad outline'}
        assert memory.scratchpad_size == 1
        memory.clear_scratchpad()
        assert memory.scratchpad_size == 0
        assert memory.read_scratchpad('repair') == ''

    # ── file mode: append / read / clear ─────────────────────────────

    def test_append_and_read_newest_last(self, file_memory):
        file_memory.write_scratchpad({'finding': 'first'})
        file_memory.write_scratchpad({'finding': 'second'})
        entries = file_memory.read_scratchpad()
        assert [entry['finding'] for entry in entries] == ['first', 'second']
        assert file_memory.scratchpad_size == 2

    def test_read_before_first_write_is_empty(self, file_memory):
        assert file_memory.read_scratchpad() == []
        assert file_memory.scratchpad_size == 0

    def test_key_value_call_wraps_into_entry(self, file_memory):
        file_memory.write_scratchpad('repair', 'bad outline')
        entries = file_memory.read_scratchpad()
        assert entries == [{'repair': 'bad outline', 'writer': 'orchestrator'}]

    def test_clear_truncates_file(self, file_memory):
        file_memory.write_scratchpad({'finding': 'gone soon'})
        file_memory.clear_scratchpad()
        assert file_memory.read_scratchpad() == []
        assert file_memory.scratchpad_size == 0

    def test_entries_persist_on_disk(self, minimal_config, tmp_path):
        path = tmp_path / 'scratchpad.jsonl'
        MemoryManager(minimal_config, scratchpad_path=str(path)).write_scratchpad({'note': 'kept'})
        reopened = MemoryManager(minimal_config, scratchpad_path=str(path))
        assert reopened.read_scratchpad() == [{'note': 'kept', 'writer': 'orchestrator'}]

    # ── writer stamping (decision 17) ────────────────────────────────

    def test_writer_defaults_to_orchestrator(self, file_memory):
        file_memory.write_scratchpad({'finding': 'x'})
        assert file_memory.read_scratchpad()[0]['writer'] == 'orchestrator'

    def test_writer_takes_flow_name(self, file_memory):
        file_memory.write_scratchpad({'finding': 'x'}, writer='compose')
        assert file_memory.read_scratchpad()[0]['writer'] == 'compose'

    def test_forged_writer_is_overwritten(self, file_memory):
        file_memory.write_scratchpad({'finding': 'x', 'writer': 'forged'}, writer='audit')
        assert file_memory.read_scratchpad()[0]['writer'] == 'audit'

    # ── read filters ─────────────────────────────────────────────────

    def test_filter_by_writer(self, file_memory):
        file_memory.write_scratchpad({'finding': 'a'}, writer='compose')
        file_memory.write_scratchpad({'finding': 'b'})
        file_memory.write_scratchpad({'finding': 'c'}, writer='compose')
        entries = file_memory.read_scratchpad(writer='compose')
        assert [entry['finding'] for entry in entries] == ['a', 'c']

    def test_filter_by_keys_present(self, file_memory):
        file_memory.write_scratchpad({'flow': 'compose', 'summary': 's', 'metadata': {}})
        file_memory.write_scratchpad({'finding': 'loose note'})
        entries = file_memory.read_scratchpad(keys=['flow', 'summary'])
        assert len(entries) == 1
        assert entries[0]['flow'] == 'compose'

    def test_filters_combine(self, file_memory):
        file_memory.write_scratchpad({'flow': 'compose', 'summary': 's'}, writer='compose')
        file_memory.write_scratchpad({'flow': 'audit', 'summary': 's'}, writer='audit')
        file_memory.write_scratchpad({'finding': 'x'}, writer='compose')
        entries = file_memory.read_scratchpad(writer='compose', keys=['flow'])
    ...
    def test_completion_record_shape(self, file_memory):
        ...
        assert file_memory.read_scratchpad() == [expected]
    ...
    def test_completion_record_default_metadata(self, file_memory):
        ...
        assert file_memory.read_scratchpad(keys=['flow', 'summary', 'metadata']) == [record]
...
def _tool_call_messages() -> list[dict]:
    ...
    return [
        ...
        {'role': 'assistant', 'content': [{'type': 'tool_use', 'id': 'toolu_01',
                                           ...
                                           'input': {'utterance': 'Draft a post about cheetahs'}}]},
...
_HOT_PATH_TOOLS = ('read_state', 'write_state', 'detect_and_fill', 'activate_flow',
...
class TestOrchestratorDispatch:
    ...
    def test_detect_and_fill_routes_to_nlu_tool_entry(self, mock_agent):
        pex = mock_agent.pex
        pex.nlu = MagicMock()
        pex.nlu.detect_and_fill.return_value = {'flow_name': 'outline', 'confidence': 0.85,
            'candidates': [{'flow_name': 'outline', 'confidence': 0.85}],
            'slots': {'source': [{'post': 'p1'}]}, 'entities': []}
        result = pex._dispatch_tool('detect_and_fill',
                                    {'text': 'make a post on agents', 'intent': 'Draft'})
        pex.nlu.detect_and_fill.assert_called_once_with(
            'make a post on agents', intent='Draft', payload=None, flow_name=None)
        assert result['_success'] is True
        assert result['intent'] == 'Draft'
        assert result['confidence'] == 0.85
        assert result['candidates'][0]['flow_name'] == 'outline'
        assert result['slots'] == {'source': [{'post': 'p1'}]}
        assert pex.flow_stack.find_by_name('outline') is None  # data only — nothing pushed

    def test_scratchpad_tools_route_to_memory(self, mock_agent, tmp_path):
        pex = mock_agent.pex
        pex.memory = MemoryManager(pex.config, scratchpad_path=str(tmp_path / 'scratch.jsonl'))
...
class TestDispatchFlow:
    ...
    def test_completion_record_is_the_tool_result(self, wired):
        ...
        assert pex.memory.read_scratchpad(keys=['flow', 'summary']) == [result['completion']]
...
class TestPolicyCompletion:
    ...
    def _migrated_policy(self, agent, summary, metadata=None):
        ...
        components = {'engineer': pex.engineer, 'memory': pex.memory, 'config': pex.config,
                      ...
                      'state_file': agent.world.state_file}
    ...
    def test_complete_flow_blocks_ungrounded_completion(self, wired):
        ...
        assert pex.memory.read_scratchpad(keys=['flow', 'summary']) == []  # no record written
...
class TestOrchestratorPrompt:
    ...
    def prompt_inputs(self, minimal_config):
        engineer = PromptEngineer(minimal_config)
        memory = MemoryManager(minimal_config)
        memory.write_preference('paragraph_length', 'shorter paragraphs')
    ...
    def test_preferences_snapshot_appears(self, prompt_inputs):
        ...
        assert '- paragraph_length: shorter paragraphs' in prompt
        # Snapshot semantics: a write AFTER the build only enters the next session's prompt.
        prompt_inputs[1].write_preference('tone', 'casual')
        assert '- tone: casual' not in prompt
...
def orch_agent(sessions_dir, monkeypatch):
    """Agent with a tmp sessions root and scripted LLM calls (orchestrator is unconditional)."""
...
class TestCompressionTrigger:
    ...
    def test_usage_recorded_including_cache_tokens(self, orch_agent):
        ...
        assert orch_agent.last_prompt_tokens == 6000
