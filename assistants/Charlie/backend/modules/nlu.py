...
class NLU:
    ...
    # ── Orchestrator tool entry (changes.md §4.1, decision 18) ─────────

    def detect_and_fill(self, user_text:str, intent:str|None=None, payload:dict|None=None,
                        flow_name:str|None=None) -> dict:
        """Tool entry for the orchestrator loop: flow detection + slot filling + entity
        extraction/repair for one utterance, WITHOUT intent classification — the orchestrator
        owns intent and may pass it as a hint to narrow flow candidates. Assumes the user turn
        is already in context (the pre-hook appends it before the loop runs).

        Pass `flow_name` to pin the flow and skip detection — the action-turn path (decision
        13): the dax already resolved the flow, so only slot filling runs, against the right
        flow's slot schema. Mirrors the old pipeline's dax short-circuit in `understand`.

        Returns data only — it never pushes flows or inserts states; the orchestrator persists
        results via write_state. Per decision 18 it does not decide ask-vs-proceed: it reports
        confidence + ranked candidates and the loop decides."""
        if flow_name:
            detection = {'flow_name': flow_name, 'confidence': 1.0,
                         'pred_flows': [{'flow_name': flow_name, 'confidence': 1.0}]}
        else:
            if intent is None:
                # Flow-detection prompts are per-intent. Without a hint, default from the
                # active flow (authoritative on a continuing turn), else 'Converse' — the same
                # fallback _classify_intent uses on failure. No intent classification here.
                active = self.flow_stack.get_flow()
                intent = active.intent if active else 'Converse'
            detection = self._detect_flow(user_text, intent)
        flow_name = detection['flow_name']

        # Fill a detached flow: a deepcopy of the stacked flow (honors prior fills on a
        # continuing turn) or a fresh instance — the live stack is never mutated.
        existing = self.flow_stack.find_by_name(flow_name)
        flow = deepcopy(existing) if existing else flow_classes[flow_name]()
        self._fill_slots(flow, payload or {})
        self._repair_entities(self.world.current_state(), flow)

        # entity_slot may legitimately be absent (e.g. Converse flows have no grounding slot).
        entity_slot = flow.slots.get(flow.entity_slot)
        return {
            'flow_name': flow_name,
            'confidence': detection['confidence'],
            'candidates': detection['pred_flows'],
            'slots': flow.slot_values_dict(),
            'entities': entity_slot.to_dict() if entity_slot else [],
        }
