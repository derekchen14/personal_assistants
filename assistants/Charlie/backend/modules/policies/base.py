...
class BasePolicy:
    ...
    def complete_flow(self, flow, state, summary:str, metadata:dict|None=None) -> dict|None:
        """The single call a policy makes at the moment its flow finishes (changes.md §8). The status
        change goes through write_state op='update_flow' (so the §6 grounding validation fires and
        state.json is rewritten) and the §5.3 completion record {flow, summary, metadata} is appended
        to the session scratchpad; activate_flow collects it via pop_completion and returns it as the
        tool result. Call it before stacking any follow-up flow — the completing flow must be top of stack."""
        ...
        self._completion = self.memory.write_completion(flow.name(), summary, metadata=metadata)
