...
# Read-only domain tools the orchestrator may call directly for trivial lookups
# (changes.md §4.3, decision 16). Every write still goes through a flow via activate_flow,
# preserving policy invariants, grounding discipline, and completion records.
...
def _block_summaries(artifact) -> list:
    ...
    for block in artifact.blocks:
        ...
        if 'title' in data:  # e.g. the audit checklist's 'Audit found N issues — pick which to fix'
...
class PEX:
    ...
    def __init__(self, config, ambiguity, engineer, memory, world):
        ...
        # Orchestrator hot-path tools (changes.md §4.1) — wiring only; the implementations
        # live in DialogueState (state file), MemoryManager (scratchpad JSONL), NLU,
        # and the policies. Agent attaches `nlu` after construction; the old
        # execute() path never reads it.
        self.nlu = None
        self._orchestrator_dispatch = {
            'read_state':           self._dispatch_read_state,
            'write_state':          self._dispatch_write_state,
            'detect_and_fill':      self._dispatch_detect_and_fill,
    ...
    # -- Orchestrator hot-path dispatch (changes.md §4.1) -------------------
    # Thin wiring onto the Phase 1/2 surfaces. Errors raised below (bad write_state op,
    # unknown flow, grounding violation) are caught by _dispatch_tool's try/except and
    # returned as corrective tool errors for the orchestrator loop to retry on.
    ...
    def _dispatch_detect_and_fill(self, params:dict) -> dict:
        """NLU as a tool (decision 18): flow detection + slot-filling for one utterance,
        returned AS DATA — confidence and ranked candidates included. Nothing is pushed;
        the orchestrator decides ask-vs-proceed and stacks the flow via write_state."""
        state = self.world.current_state()
        if state.grounding['post'] and not state.active_post:
            state.active_post = state.grounding['post']
        result = self.nlu.detect_and_fill(params['text'], intent=params.get('intent'),
                                          payload=params.get('payload'),
                                          flow_name=params.get('flow_name'))
        return {'_success': True, 'intent': FLOW_CATALOG[result['flow_name']]['intent'], **result}

    def activate_flow(self, params:dict) -> dict:
        """Run the named flow's policy inline (changes.md §4.1) — the Hermes delegate_task
        analogue. Grounding comes from the state file's grounding block; _security_check and
        _validate_artifact re-attach around the policy run. On completion the flow's
        completion record (decision 7) is written via MemoryManager and returned as the tool
        result. State-file persistence stays with write_state (the orchestrator epilogue)."""
        state = self.world.current_state()
        ...
        record = policy.pop_completion()  # set when the policy completed via complete_flow (§5.3)
        self.world.insert_artifact(artifact)
        if state.active_post:  # changes.md §6: the grounding block stays authoritative
        ...
        if record is None:  # transitional: policy not yet migrated to complete_flow
            summary = artifact.thoughts or f'{flow.name()} completed'
            record = self.memory.write_completion(flow.name(), summary, metadata=artifact.data)
    ...
    def _stage_flow(self, state, flow_name:str):
        ...
        from the state file's flow_stack block when an entry exists (changes.md §5.2).
    ...
    def _dispatch_append_scratchpad(self, params:dict) -> dict:
        self.memory.write_scratchpad(params['entry'])  # `writer` stamped by code (decision 17)
        return {'_success': True, 'size': self.memory.scratchpad_size}

    def _dispatch_read_scratchpad(self, params:dict) -> dict:
        entries = self.memory.read_scratchpad(writer=params.get('writer'), keys=params.get('keys'))
        return {'_success': True, 'entries': entries}

    def _dispatch_store_preference(self, params:dict) -> dict:
        """Persist a durable user preference to L2 (the converted `preference` flow)."""
        self.memory.write_preference(params['key'], params['value'])
    ...
    def get_tools_for_orchestrator(self) -> list[dict]:
        """The orchestrator's tool list (changes.md §4): hot-path dedicated tools, the three
        long-tail component dispatch tools, and the read-only domain allowlist (decision 16).
        call_flow_stack is deliberately absent — its job lives in read_state/write_state."""
        ...
        return [
            ...
            },
            {
                ...
                'description': (
                    ...
                    "(slot-name → value, in the exact shapes detect_and_fill returns: strings "
                    "for single-value slots, lists for multi-value slots), `stage`, and "
            ...
            {
                'name': 'detect_and_fill',
                'description': (
                    "Run fine-grained NLU over one user utterance: detect the best flow in the "
                    "16-flow catalog and fill its typed slots (entity extraction included). "
                    "Returns flow_name, intent, confidence, ranked candidates, and the filled "
                    "slot values AS DATA — it does not push the flow. To proceed: write_state "
                    "op=stackon, then write_state op=update_flow with fields.slots set to the "
                    "returned `slots` object VERBATIM (same keys, same values — they are typed "
                    "and already validated; never rename, drop, or reinterpret them), then "
                    "activate_flow. Ask the user (handle_ambiguity) instead when confidence is "
                    "low or the top candidates are close. Pass `intent` (your own "
                    "classification) to narrow the flow candidates; on an action turn, pass the "
                    "turn's `payload` too so clicked options unpack into slots, AND pass "
                    "`flow_name` set to the turn's resolved flow — detection is skipped and "
                    "slots fill against that flow's own schema."
                ),
                'input_schema': {
                    'type': 'object',
                    'properties': {
                        'text': {'type': 'string', 'description': "the user's utterance"},
                        'intent': {'type': 'string',
                                   'description': 'intent-family hint to narrow candidates'},
                        'payload': {'type': 'object',
                                    'description': 'optional frontend payload for this turn'},
                        'flow_name': {'type': 'string',
                                      'description': 'pin the flow (action turns) — skips '
                                                     'detection, fills this flow\'s slots'},
                    },
                    'required': ['text'],
                },
            },
            ...
            },
            {
                ...
                'description': (
                    ...
                    "keeping, working notes (also the home of the converted `store` flow). The "
                    "`writer` stamp is added by code; do not include a writer key yourself."
                ),
                'input_schema': {
                    'type': 'object',
            ...
            },
            {
                ...
                'description': (
                    ...
                    "the session (the converted `preference` flow). Pass the preference `key` and "
                    "its `value`."
                ),
                'input_schema': {
                    'type': 'object',
