OUTLINE_PROMPT = {
    ...
    'slots': (
        ...
        "### steps (optional)\n\n"
        "Type: ChecklistSlot. Grafted from compose: an ordered list of process-level actions for "
        "writing each section. Each item is `{name: <step>, description: ''}`. Fires on numbered or "
        "bulleted lists of writing actions. Do NOT use for content-point references — those are "
        "sections on the post.\n\n"
        "### feedback (optional)\n\n"
        "Type: FreeTextSlot. Grafted from compose: qualitative writing direction — tone, length, "
        "angle, audience. Also the outline → prose hand-off phrase ('flesh it out'). Leave null "
        "when the user supplies only structure."
    ),
...
REFINE_PROMPT = {
    ...
    'slots': (
        ...
        "a list.\n\n"
        "### image (optional)\n\n"
        "Type: ImageSlot. Grafted from add: an image to insert. Fills when the user asks to add or "
        "insert an image, diagram, or screenshot. Image type comes from the user's word.\n\n"
        "### position (optional)\n\n"
        "Type: PositionSlot. Grafted from add: 0-based index for where to insert the new content. "
        "'Right after the intro' → 1; 'at the start' → 0. Leave null when placement is unspecified.\n\n"
        "### settings (optional)\n\n"
        "Type: DictionarySlot. Grafted from tidy: key-value formatting rules the user specifies "
        "('H2 headings' → `{headings: 'H2'}`). Leave null on bare requests — the policy applies defaults."
    ),
    'examples': '''<positive_example>
## Conversation History
