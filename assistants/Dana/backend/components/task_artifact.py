...
VALID_BLOCK_TYPES = frozenset(('card', 'checklist', 'confirmation', 'toast', 'default',
                               'selection', 'list', 'grid', 'compare', 'chart', 'table'))
...
class TaskArtifact:
    ...
    Internal storage is `parts: list[Part]`. The constructor accepts `parts={dict}`
    (wrapped as a data Part) plus `thoughts`/`code` (each wrapped as a text Part
    tagged via Part.metadata.kind). Consumer-facing properties `artifact.thoughts`
    and `artifact.code` unpack from parts.

    Dana retains the legacy single-block compat API (`set_artifact(block_type, data,
    source=, ...)`, `artifact.data` = last block's data, `artifact.block_type`,
    `artifact.source`, `artifact.display_name`) until callers migrate. The classification
    dict (source/display_name and any other key/values written via `update_data`) lives
    inside a data Part in `self.parts`."""

    def __init__(self, config=None, origin:str='', parts:dict={}, blocks:list=[],
                 code:str|None=None, thoughts:str=''):
        # `config` accepted positionally for legacy `TaskArtifact(self.config)` callers; ignored.
        self.origin = origin
        self.parts = []
    ...
    def set_artifact(self, *args, **kwargs):
        # Legacy: set_artifact(block_type, data, source=, display_name=, code=, panel=)
        if args and isinstance(args[0], str) and (len(args) >= 2 or 'data' in kwargs):
            ...
            if display_name:
                self.update_data(display_name=display_name)
            if code:
                self.code = code
            return
        # New: set_artifact(origin=, blocks=, new_data=)
    ...
    @property
    def data(self) -> dict:
        """Legacy: last block's data dict. New consumers should iterate `parts` directly or
        use `update_data()` for classification keys."""
    ...
    @property
    def source(self) -> str:
        return self._data_part().get('source', '')

    @property
    def display_name(self) -> str:
        return self._data_part().get('display_name', '')

    def _data_part(self) -> dict:
