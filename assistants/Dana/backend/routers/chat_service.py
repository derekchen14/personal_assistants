...
from backend.utilities.services import DatasetService
...
async def chat(websocket: WebSocket):
    ...
    try:
        ...
        datasets_result = dataset_svc.list_datasets()
        dataset_items = datasets_result.get('result', [])

        welcome_frame = {
            'type': 'list',
            'show': True,
            'data': {'title': 'Your Datasets', 'items': dataset_items},
            'source': 'welcome',
            'panel': 'top',
        } if dataset_items else None
        ...
        while True:
            try:
                ...
                if body.get('reset'):
                    ...
                    dataset_svc_r = DatasetService()
                    datasets_r = dataset_svc_r.list_datasets()
                    items_r = datasets_r.get('result', [])
                    reset_frame = {
                        'type': 'list',
                        'show': True,
                        'data': {'title': 'Your Datasets', 'items': items_r},
                        'source': 'welcome',
                        'panel': 'top',
