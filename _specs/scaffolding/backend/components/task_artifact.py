...
class TaskArtifact:
    ...
    (each wrapped as a text Part tagged via Part.metadata.kind='thoughts'|'code').
    Consumer-facing properties `artifact.thoughts`, `artifact.code`, `artifact.data`
    unpack from parts so callers don't see Part objects directly. `blocks` are
    visual BuildingBlocks routed to panels by RES.

    No new top-level attributes without explicit approval. Domain-specific data
    lives inside `blocks[].data` or the artifact's parts."""
