...
- YAML holds system-level and domain-level runtime config: environment, models, persona, guardrails, session, memory, resilience, context_window, logging, display, thresholds, feature flags, response, human_in_the_loop, slot types, key entities
...
| Goes in `ontology.py` | Goes in YAML config (§1–§15) |
|---|---|
| Intent enum (Plan, Converse, Clarify, Read, Prepare, Transform, Schedule) | §1 Models (provider, parameters, cost budgets) |
...
| — | §12 Response constraints (length bounds, language, citation; voice/composition lives in PEX's prompt) |
| — | §13 Human-in-the-loop (tool approval, escalation) |
| — | §14 Slot types (custom validators per domain) |
| — | §15 Key entities (domain-specific entity names) |
...
| 1 | `models` | `provider: anthropic`, `model_id: claude-sonnet-4-5-latest`, `temperature: 0.0`, `top_p: 1.0`, `max_output_tokens: 4096`, `stop_sequences: []`, overrides: `skill: { model_id: claude-opus-4-6 }`, all cost budgets `null` (unlimited), `warn_threshold: 0.8` |
...
| 12 | `response_constraints` | `min_tokens: 10`, `max_tokens: 2048` for all intents, `language: en`, `supported_languages: [en]`, `confidence_auto_threshold: 0.6`, `citation_mode: footnote`; voice/composition is not config — PEX composes the reply directly via a voice Skill in its system prompt (no templates; see [PEX](../modules/pex.md)) |
| 13 | `human_in_the_loop` | mode `none`, `timeout_ms: 60000`, escalation disabled |
| 14 | `slot_types` | None (base types in ontology.py; custom always domain-specific) |
| 15 | `key_entities` | None (always domain-specific) |
...
persona:
  ...
  # Consumed by: Prompt Engineer (system prompt composition), PEX response composition (formatting)
...
guardrails:
  ...
  #   PEX / main-Agent post-hook (output filtering),
...
memory:
  ...
  # Consumed by: MEM (all tiers),
  #   NLU/PEX/MEM (indirectly via memory contents),
...
logging:
  ...
  signal_export:
    enabled: false                  # export evaluation signals
...
thresholds:
  stream_threshold_tokens: 200      # above this, PEX streams the response
                                    # (ref: pex.md § Task Artifact & rendering)
...
response_constraints:               # output boundaries (one flat layer)
  length_bounds:
    default:
      min_tokens: 10
      max_tokens: 2048
  ...
  # Voice/composition is NOT config: PEX composes the reply directly via a voice Skill loaded into its
  # system prompt (no template registry), picking a block type from the artifact's data shape; the
  # streaming threshold lives in thresholds.stream_threshold_tokens (ref: pex.md § Task Artifact).
  # Consumed by: PEX response composition (block selection + length enforcement),
  #   Self-check gate (length bounds check), NLU (confidence threshold),
...
human_in_the_loop:
  ...
  # Consumed by: PEX `check()` / `activate_flow` (tool gating),
  #   main Agent (escalation routing),
  #   Server Setup (webhook/queue integration)

# No shared defaults for: §14 slot_types, §15 key_entities
...
response_constraints:               # override only what differs from shared defaults
  citation_mode: inline             # voice itself is not config — it's a Skill in PEX's system prompt
```

### Section Details

1. **`models`**: LLM provider and model selection. The `default` block sets the primary model (provider, model_id, temperature, top_p, max_output_tokens, stop_sequences). The `overrides` map allows named call sites (nlu_vote, self_check, skill) to use different models or parameters without code changes. The `cost` sub-section co-locates token budgets (per-session, per-turn, daily) with model config because token budgets are directly tied to model usage. Model selection is the single highest-impact tunable; every framework makes this configurable.
...
9. **`display`**: Display type list, chart type list, page size, and accepted file formats (with MIME types). Display types govern how PEX maps the Task Artifact to blocks. File formats define what the domain accepts for upload (e.g., `[csv, xlsx, tsv]` for data analysis, `[md, docx]` for blogging). Shared defaults provide minimal baseline.
10. **`thresholds`**: Numeric thresholds controlling runtime behavior. `stream_threshold_tokens` for PEX response streaming. `nlu_confidence_min` and `nlu_vote_agreement_min` for NLU classification confidence. `ambiguity_escalation_turns` caps the ambiguity loop. `scratchpad_promotion_frequency` controls how many times a snippet must recur to trigger promotion to long-term memory.
...
12. **`response_constraints`**: Output boundaries, one flat layer — length bounds (min/max tokens, with per-intent overrides), language (ISO 639-1 code), supported languages list, confidence auto-threshold (below which the agent asks clarification instead of auto-responding), and citation mode; the self-check gate already checks length bounds — this section gives it concrete values to read. **Voice/composition is not config**: there is no template registry — PEX composes the text directly using a voice Skill loaded into its system prompt, and picks a block type from the artifact's data shape; the streaming threshold lives in `thresholds.stream_threshold_tokens`. Reference: [PEX](../modules/pex.md#task-artifact--rendering), [Building Blocks — Block Selection](../utilities/blocks.md#block-selection)
13. **`human_in_the_loop`**: Approval and escalation configuration. Tool approval mode (none/non_idempotent/all/explicit_list) gates which actions require human approval before execution, with a timeout. Escalation routing (triggers, channel, endpoint) determines when and how to hand off to a human agent. As agents perform consequential actions, the ability to gate specific tools without code changes is essential.
14. **`slot_types`**: Custom slot type validators per domain. Base types (string, number, boolean, enum) in ontology.py. Always domain-specific.
15. **`key_entities`**: Entity names grounding the Ambiguity Handler's Partial level. Always domain-specific.
...
| 12 | `response_constraints` | length bounds `min_tokens` < `max_tokens`, language codes valid ISO 639-1, `confidence_auto_threshold` 0.0–1.0, `citation_mode` in `[none, inline, footnote]` |
| 13 | `human_in_the_loop` | Mode in `[none, non_idempotent, all, explicit_list]`; `explicit_list` entries are non-empty strings; `timeout_ms` > 0 |
| 14 | `slot_types` | Validator functions exist and are importable |
| 15 | `key_entities` | Entity names are non-empty strings |
...
| PEX `activate_flow` | Policy mapping, tool approval, retries | `config.flows[dact].policy`, `config.resilience.tool_retries`, `config.human_in_the_loop.tool_approval` |
| PEX `check()` pre-hook | Flow validation | `config.flows` |
| PEX recovery | Retry config | `config.resilience` |
| PEX response composition (voice Skill) | Streaming threshold, length bounds, language | `config.thresholds`, `config.response_constraints` |
| PEX (Task Artifact) | Display types, chart types, page size | `config.display` |
| main Agent post-hook | Output filtering, guardrails | `config.guardrails` |
| Task Artifact | Page size for pagination | `config.display.page_size` |
...
| MEM | Summarization triggers, user-prefs backend, RAG params, promotion frequency | `config.memory`, `config.thresholds.scratchpad_promotion_frequency` |
